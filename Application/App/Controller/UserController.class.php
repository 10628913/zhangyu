...
class UserController extends AppController {
    ...
    public function sendSmsByForgetPassword(){
        ...
        $whereAdmin["mobile"] = $mobile;
        $whereAdmin["is_show"] = "1";        
        $adminInfo = $this->adminDb->where($whereAdmin)->find();
        if($adminInfo){
    ...
    public function login()
    {
        ...
        if(!$info){
            ...
            if($password == $info["password"]){
                ...
                $infoData["avatar"] = thumb($infoData["avatar"],200,200);
    ...
    public function register()
    {
        ...
        $whereAdmin["is_show"] = "1";
        //判断用户名是否存在
        $isIn = $this->adminDb->where($whereAdmin)->find();
        if(!$isIn)
        {
            //不存在
            ...
            {   
                ...
                $userInfo = $this->adminDb->where($where)->field("password,encrypt",true)->find();
                $this->httpResponse(1,"注册完成",$userInfo);
            }else
            {
    ...
    public function addressAdd(){
        ...
        $data["uid"] = $this->adminInfo["uid"];
        //发货人
        $data["consigner"] = I("consigner");
        //发货人手机号
        $data["consigner_mobile"] = I("consigner_mobile");
        //公司
        ...
        $where["uid"] = $data["uid"];
        $info = $this->adminDb->where($where)->field("consigner,consigner_mobile")->find();
        if($info["consigner"]){
        ...
        if(!$data["consigner_mobile"]){
            $this->httpResponse(-1,"请输入手机号");
        }
        if(!$data["address"]){
            $this->httpResponse(-1,"请输入详细地址");
        }
        $result = $this->adminDb->where($where)->save($data);
        if($result){
            $ret["code"] = 1;
            $ret["msg"] = "添加成功";
            $this->ajaxReturn($ret);
    ...
    public function userAddressList(){
        ...
        $data = $this->adminDb->where($where)
        ->field("uid,consigner,consigner_mobile,province,city,county,district,town,address,zipcode,company,remark")
        ->find();
        if(!$data["consigner"]){
            $this->httpResponse(-1,"无发货地址");
        }
        if(!$data["consigner_mobile"]){
            $this->httpResponse(-1,"无发货地址");
        }
